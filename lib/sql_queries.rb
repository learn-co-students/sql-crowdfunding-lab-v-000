...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_construction_category
  "SELECT projects.category, pledges.amount FROM projects
  JOIN pledges ON projects.ID = pledges.project_id
  WHERE projects.category = 'construction';"
end

def selects_the_category_name_and_the_sum_total_of_the_all_its_pledges_for_the_lifestyle_category
  "SELECT projects.category, SUM(pledges.amount) AS amount FROM projects
  JOIN pledges ON projects.id = pledges.project_id
  GROUP BY projects.category = 'lifestyle'
  LIMIT 1 OFFSET 1;"
end
