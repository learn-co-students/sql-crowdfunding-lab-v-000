...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_name
  "SELECT title, SUM(pledges.amount) FROM projects INNER JOIN pledges WHERE projects.id = pledges.project_id GROUP BY title ORDER BY title ASC"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT name, age, SUM(pledges.amount) FROM pledges INNER JOIN users ON pledges.user_id = users.id GROUP BY name ORDER BY name ASC"
end
