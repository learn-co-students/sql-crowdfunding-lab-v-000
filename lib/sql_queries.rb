...
"SELECT projects.title, (SUM(pledges.amount) - funding_goal) AS amount_over_funded FROM projects
 INNER JOIN pledges
 ON projects.id = pledges.project_id 
 GROUP BY projects.title 
 HAVING amount_over_funded >= 0
 ;" 
end
...
"SELECT projects.category, pledges.amount AS amount
FROM projects
INNER JOIN pledges
ON projects.id = pledges.project_id
