...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS remaining_balance FROM projects
JOIN pledges
ON projects.id = pledges.project_id GROUP BY projects.id HAVING remaining_balance >= 0;"
end
