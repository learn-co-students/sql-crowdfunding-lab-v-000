...
"SELECT title, sum(amount) - funding_goal AS difference FROM projects INNER JOIN pledges ON projects.id=pledges.project_id GROUP BY title HAVING 0 <= difference;"
end
