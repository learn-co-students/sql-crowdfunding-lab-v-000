...
"SELECT projects.title, SUM(pledges.amount)-projects.funding_goal AS over FROM projects INNER JOIN pledges ON pledges.project_id = projects.id GROUP BY pledges.project_id HAVING over >= 0;"
end
