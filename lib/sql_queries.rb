...
"SELECT projects.title, projects.funding_goal - SUM(pledges.amount) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title ORDER BY projects.funding_goal LIMIT 2"
end
