...
"SELECT title, SUM(amount) - funding_goal AS amount_over_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY title HAVING amount_over_goal >= 0;"
end
