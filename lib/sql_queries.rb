...
"SELECT users.name, SUM(pledges.amount) as amount_pledged FROM users INNER JOIN pledges on users.id = pledges.user_id GROUP BY users.name ORDER BY amount_pledged, users.name;"
end
