...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) * -1 AS goal_over FROM projects
JOIN pledges
...
"SELECT users.name, SUM(pledges.amount) as tot_amt FROM users
JOIN pledges
ON pledges.user_id = users.id
GROUP BY users.name
ORDER BY tot_amt, users.name"
end
