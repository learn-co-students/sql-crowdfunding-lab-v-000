...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS t_amount FROM projects
JOIN pledges ON project_id = projects.id
GROUP BY title
HAVING t_amount >= 0"
end
...
"SELECT users.name, SUM(pledges.amount) AS t_amount FROM pledges
JOIN users ON user_id = users.id
GROUP BY name
ORDER BY t_amount, users.name"
end
