...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS overfund FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING overfund >= 0"
end
