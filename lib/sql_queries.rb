...
"SELECT projects.title, (SUM (pledges.amount)-projects.funding_goal) AS goal_met FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING goal_met >= 0 ;" 
end
...
"SELECT projects.category, SUM (pledges.amount) FROM projects INNER JOIN pledges on projects.id = pledges.project_id WHERE projects.category = 'books' GROUP BY projects.category HAVING SUM (pledges.amount);"
end
