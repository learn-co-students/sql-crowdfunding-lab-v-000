...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS surplus FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING surplus >= 0;"
end
