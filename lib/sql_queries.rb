...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS surplus FROM projects
  JOIN pledges ON pledges.project_id = projects.id
  GROUP BY projects.title HAVING surplus >= 0;"end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) FROM users
  INNER JOIN pledges on users.id = pledges.user_id
  GROUP BY name
  ORDER BY SUM(amount), name;"end

def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
"SELECT Projects.category, Pledges.amount FROM pledges INNER JOIN Projects ON Pledges.project_id = Projects.id WHERE Projects.category = 'music';"end
