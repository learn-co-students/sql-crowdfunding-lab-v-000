...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS balance FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING balance <= 0;"
end
