...
"SELECT title, SUM(pledges.amount) AS pledge_amounts FROM projects JOIN pledges ON projects.id=pledges.project_id GROUP BY projects.title;"
end
...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS funds_over_goal FROM projects JOIN pledges ON projects.id=pledges.project_id GROUP BY projects.title HAVING
