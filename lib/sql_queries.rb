...
def selects_the_titles_of_all_projects_and_their_pledge_amounts_alphabetized_by_name
  "SELECT projects.title, SUM(pledges.amount) AS pledged_total FROM projects
...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
  "SELECT users.name, users.age, SUM(pledges.amount) AS pledged_total FROM users
    LEFT OUTER JOIN pledges
...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
  "SELECT users.name, SUM(pledges.amount) AS pledged_total FROM users
    LEFT OUTER JOIN pledges
    ON users.id = pledges.user_id
    GROUP BY users.name
    ORDER BY pledged_total ASC;"
end
