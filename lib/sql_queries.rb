...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS remaining FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING remaining <= 0;"
end
