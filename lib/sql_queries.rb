...
"SELECT title, (SUM(pledges.amount) - projects.funding_goal) AS overflow FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING overflow >= 0"
end
