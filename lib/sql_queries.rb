...
"SELECT projects.title, SUM(pledges.amount)
 ...
 GROUP BY projects.title;" #We want to use GROUP By because of the aggregate function SUM #
end
...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_left
 ...
 JOIN pledges
 ON projects.id = pledges.project_id
 GROUP BY projects.title HAVING amount_left >= 0;" #HAVING clause can be used with aggregates #
end
