...
"SELECT users.name, sum(pledges.amount) AS pledge_amount FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledge_amount ASC;"
end
