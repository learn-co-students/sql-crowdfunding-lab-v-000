...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS amount_over_goal FROM projects INNER JOIN pledges ON pledges.project_id = projects.id GROUP BY projects.title HAVING SUM(pledges.amount) >= projects.funding_goal;"
end
