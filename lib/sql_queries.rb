...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges
  "SELECT Users.name, Users.age, SUM(Pledges.amount) FROM Pledges INNER JOIN Users ON Pledges.user_id = Users.id GROUP BY Users.name, Users.age;"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT Projects.title, (SUM(Pledges.amount) - Projects.funding_goal) AS goal_diff FROM Projects INNER JOIN Pledges ON Projects.id = Pledges.project_id GROUP BY Projects.title, Projects.funding_goal HAVING goal_diff >= 0;"
end
