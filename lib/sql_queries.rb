...
"SELECT name, SUM(amount) FROM users JOIN pledges ON users.id = pledges.user_id JOIN projects ON projects.id = pledges.project_id GROUP BY name ORDER BY SUM(amount), name;"
end
