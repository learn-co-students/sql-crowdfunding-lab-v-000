...
"SELECT projects.title, SUM(pledges.amount) 
 FROM projects LEFT OUTER JOIN pledges ON projects.id = pledges.project_id
 GROUP BY projects.title ORDER BY projects.title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges
"SELECT users.name, users.age, SUM(pledges.amount)
 FROM pledges LEFT OUTER JOIN users ON pledges.user_id = users.id
 GROUP BY users.name;"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM pledges
 LEFT OUTER JOIN projects ON projects.id = pledges.project_id
