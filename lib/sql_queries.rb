...
"SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS leftover FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING leftover <= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount
  "SELECT users.name, SUM(pledges.amount) AS pledge_amounts FROM users LEFT OUTER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount);"
end
