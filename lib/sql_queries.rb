...
# the = "equal sign" is different in such that it looks for matching colums in different tables based on id.

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT users.name, users.age, SUM(pledges.amount) FROM users
INNER JOIN pledges ON users.id = pledges.user_id GROUP BY name"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
"SELECT projects.title, SUM(pledges.amount) - (projects.funding_goal) FROM projects
INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY title
HAVING SUM(pledges.amount) >= funding_goal"
end

# "."is to target a colum assiged to or within a TABLE...
...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
 "SELECT projects.category,pledges.amount
   ...
   #notes: Music in this case is a string.
end
...
"SELECT projects.category,SUM(pledges.amount)
  ...
  #notes: Music in this case is a stri
end
