...
LEFT OUTER JOIN pledges ON pledges.user_id = users.id 
GROUP BY(users.name)
ORDER BY(users.name);"
end

def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS 'Amount Over Goal' FROM projects 
...
"SELECT users.name, SUM(pledges.amount) AS 'Amount Pledged' FROM users
