...
"SELECT title, SUM(amount) - funding_goal AS amount_over_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY title ORDER BY amount_over_goal LIMIT 2 OFFSET 8;"
end
